...
export default function KitchenOrdersApp() {
  ...
  useEffect(() => {
    ...
    loadState().then((s) => {
      if (mounted) {
        const safeState = s && s.events ? s : emptyState();
        setState(safeState);
        setLoaded(true);
    ...
    const poll = setInterval(async () => {
      ...
      if (mounted) {
        const syncedState = { ...(fresh || emptyState()), currentDate: stateRef.current.currentDate };
  ...
  const clearMenu = useCallback(() => {
    if (!window.confirm("⚠️ Очистить ВСЁ меню для ВСЕХ дат?\n\nОдновременно с меню удалятся выбранные позиции с кассы.")) return;
...
function Header({ tab, setTab, onReset, orders, currentDate, eventDates, onSwitchDate, onShowDateModal, showDateModal, onCloseModal, newDateInput, setNewDateInput, onCreateNewDate }) {
  ...
  return (
    <div style={{ padding: "0 22px" }}>
      ...
      <div style={{ marginTop: 20 }}>
        <button onClick={onClear} style={{ padding: "8px 16px", background: COLORS.redSoft, color: COLORS.red, border: `1px solid ${COLORS.red}`, borderRadius: 4, cursor: "pointer", fontFamily: "'IBM Plex Mono', monospace", fontSize: 12 }}>Очистить всё меню</button>
      </div>
    </div>
  );
}
